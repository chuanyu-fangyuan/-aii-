...
| 场景 | 方法 | 结果 |
|---|---|---|
| 真实数据获取 | `python fetch.py` | ✅ 4 源成功，入库 81 条 |
| 重复导入幂等 | 连续运行两次 | ✅ 第二次全部 `inserted=0` |
| 单源失败容错 | `python fetch.py --fail-source techcrunch` | ✅ 该源标记 `failed`，其他源正常，旧数据 81 条保留 |
...
- 问题定位：初版关键词含大量噪声（`Abstract`、`across` 等），定位为「高频词统计混入摘要文本」，改为只从标题统计高频词 + 扩充停用词后解决。
