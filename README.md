...
定时任务配置见 `.github/workflows/update.yml`：cron `20 */3 * * *`，即每 3 小时一次（北京时间 08:20 / 11:20 / 14:20 / 17:20 / 20:20 / 23:20 / 02:20 / 05:20，全部避开整点高负载时段）+ `workflow_dispatch` 手动触发。触发时机与运行结果以 **Actions 页面的运行记录**（Event 列显示 `Schedule` 或 `workflow_dispatch`）为准。
...
| 定时自动更新 | GitHub Actions cron | ⚠️ **尚未验证通过**。首版（UTC 00:17）与第二版（UTC 02:20 / 14:20）配置经 API 核验，`event=schedule` 运行数**始终为 0**——即定时事件一次都未触发，所有运行均来自 `push` 或手动。已改为 `20 */3 * * *`（每天 8 次）继续观察，详见「案例四」 |
...
- 仍未解决的部分：**截至本次提交，定时触发尚未出现过一次**。这一点在验证记录中如实保留为「未验证通过」，不使用「配置完成」冒充「运行通过」。若后续仍无 `Schedule` 事件，将改用外部定时器调用 GitHub API 触发 `workflow_dispatch` 作为兜底。
